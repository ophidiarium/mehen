...
pub struct WordingReport {
    ...
    pub weasel_density: f64,
    pub wordy_density: f64,
    pub adverb_density: f64,
    pub nominalization_density: f64,
    pub expletive_count: u64,
    pub lexical_illusions: u64,
    pub cliche_density: f64,
    pub nonword_count: u64,
    pub long_sentence_count: u64,
    pub wording_quality_score: f64,
}

pub fn analyze(sents: &[String], words_per_sent: &[Vec<String>]) -> WordingReport {
...
fn weasel_density(words: &[String], sents: &[String]) -> f64 {
    ...
    let suppressed: HashSet<usize> = sents
        ...
        .enumerate()
        .filter_map(|(i, s)| {
            if s.contains(crate::prose::lang_detect::INLINE_CODE_SENTINEL) {
                Some(i)
            } else {
                None
            }
