...
//!
//! Phase 1 scope: skeleton with `tree-sitter-kotlin` (the
//! `tree-sitter-kotlin-sg` crate, aliased as `tree-sitter-kotlin` in the
//! workspace's dependency table) wired through `LanguageAnalyzer`.
//! Tree-sitter is the 1.0 backend per the rewrite plan §6.1.
